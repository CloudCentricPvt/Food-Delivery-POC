...
fun OrderListScreen(navController: NavHostController) {
    ...
    Scaffold(
        topBar = { TopAppBar(title = { CommonUtils().CommonText("Orders", fontSize = 14) }) }
    ) { padding ->
        when {
            isLoading -> {
                Box(
...
@RequiresApi(Build.VERSION_CODES.O)
fun parseOrderDateTime(order: SendOrder, dateTimeFormatter: DateTimeFormatter): LocalDateTime {
